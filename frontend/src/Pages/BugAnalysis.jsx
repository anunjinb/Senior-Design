  ...
  return (
    <div className="ba-container fade-in">
      ...
      <div className="ba-input-wrapper">
        ...
        <input type="text" placeholder="Describe the bug..." className="ba-input" value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAnalyze()} />
        {query && !analyzing && ( <button onClick={handleClear} className="icon-btn" style={{marginRight: 8}}><XCircle size={18} /></button> )}
      ...
      <div className="fade-in" style={{marginTop: 30, textAlign: 'center', marginBottom: prediction ? 20 : 0}}>
          <p style={{fontSize: 11, fontWeight: 800, color: 'var(--text-sec)', marginBottom: 16, display:'flex', alignItems:'center', justifyContent:'center', gap:6, letterSpacing: 0.5}}>
          ...
          <div style={{display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center', maxWidth: 650, margin: '0 auto'}}>
              {SAMPLE_BUGS.map((sample, i) => (
                  <button key={i} onClick={() => handleAnalyze(sample)} className="sys-btn outline" style={{ borderRadius: '20px', fontSize: 13, padding: '8px 18px', display:'flex', alignItems:'center' }}>
      ...
      {error && <div style={{ color: 'var(--danger)', marginTop: 20, textAlign: 'center', fontWeight: 'bold' }}>{error}</div>}
           ...
           <div className="ba-card">
            ...
            {!feedbackSent && (
                <div style={{marginTop: 24, paddingTop: 20, borderTop: '1px dashed var(--border)'}}>
                    <p style={{fontSize: 11, fontWeight: 700, color: 'var(--text-sec)', marginBottom: 12, textTransform: 'uppercase', letterSpacing: 0.5}}> Is this accurate? </p>
                    <div style={{display: 'flex', gap: 12}}>
                        <button onClick={() => sendFeedback(true, prediction.label)} className="sys-btn" style={{ flex: 1, background: 'var(--success)' }}><ThumbsUp size={16}/> Yes</button>
                        <button onClick={() => { const correct = prompt("What should the severity be? (S1, S2, S3)"); if(correct) sendFeedback(false, correct.toUpperCase()); }} className="sys-btn" style={{ flex: 1, background: 'var(--danger)' }}><ThumbsDown size={16}/> No</button>
                    </div>
                </div>
            )}
            {feedbackSent && (
                <div style={{marginTop: 20, padding: 12, background: 'var(--hover-bg)', borderRadius: 8, textAlign: 'center', fontSize: '13px', color: 'var(--success)', fontWeight: '700', display: 'flex', alignItems:'center', justifyContent:'center', gap: 8}}>
          ...
          <div className="ba-card">
            ...
            <div className="custom-scrollbar" style={{maxHeight: 400, overflowY: 'auto', paddingRight: 4}}>
              {!duplicates || duplicates.length === 0 ? (
                  <div style={{padding: '40px', textAlign: 'center', color: 'var(--text-sec)', fontSize: '14px', display:'flex', flexDirection:'column', alignItems:'center', gap:10}}>
                      <Search size={32} style={{opacity:0.2}}/> No similar bugs found.
                  </div>
              ) : (
